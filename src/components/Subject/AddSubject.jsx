...
import { createSubject, deleteSubjectById, getAllSubjects } from '../../api/subject';
...
function AddSubject() {
    ...
    const fetchSemesters = async () => {
        try {
            ...
            toast.error(error?.response?.data?.message || 'Fail to load semesters');
        } finally {
            setIsFetching(false);
        }
    };
    ...
    const fetchSubjects = async () => {
        try {
            ...
            toast.error(error?.response?.data?.message || 'Fail to load subjects');
        } finally {
            setIsFetching(false);
        }
    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.success(formData.id ? 'Subject updated!' : 'Subject added!');
            setFormData({ id: null, name: '', description: '', isActive: true, semesterId: null });
            fetchSubjects();
        } catch (error) {
            toast.error(Object.values(error?.response?.data?.data || {})[0] || 'Failed to save subject');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen mt-8 flex flex-col items-center justify-start dark:bg-slate-950 px-4">
            ...
            <div className="w-full max-w-3xl mt-10">
                <h3 className="text-xl font-semibold text-gray-800 dark:text-white text-center">All Subjects</h3>
                ...
                {isFetching ? (
                    ...
                    <div className="overflow-x-auto shadow-lg border rounded-lg border-indigo-200 dark:border-indigo-700">
                        <table className="w-full border-collapse rounded-lg">
                            ...
                            <tbody>
                                {subjects.map((sub) => (
                                    <tr key={sub.id} className="text-center dark:text-gray-300">
                                        <td className="p-2 border">{sub.id}</td>
                                        <td className="p-2 border">{sub.name}</td>
                                        <td className="p-2 border">{sub.description}</td>
                                        <td className="p-2 border">{sub.semester?.name || 'N/A'}</td>
